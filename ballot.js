...
app.use(function (req, res, next) {
  if (!("token" in req.cookies)) {
    res.cookie('token', guid(), { maxAge: 900000, httpOnly: true });
  } 
  next();
});
  ...
  if (open) {
    ...
    if (x >= 0 && x < ballot_length) {
      ...
      for (var i = 0; i < sockets.length; i++) {
  			sockets[i].emit('count', {'count': c, 'oo': oo});
  		}
      res.send('vote cast');
    }  else {
...
io.on('connection', function(socket){ sockets.push(socket); console.log('sock')});
